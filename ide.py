...
try:
    ...
    # Allow running directly: python qude/ide.py
...
class QudeIDE:
    def __init__(self) -> None:
        ...
        # Quick sender (Kısayol Yollayıcı) devre dışı
        self.quick_win: tk.Toplevel | None = None
        self.quick_entry: tk.Entry | None = None
    ...
    def _show_help(self) -> None:
        ...
        yazi = (
            ...
            "  text1.size = 15 | text1.fnt.sz = 15 | text1.fsz = 15\n\n"
            "Arkaplan:\n"
        ...
        buton = (
            ...
            "  button1.size = 15 | button1.fnt.sz = 15 | button1.fsz = 15\n"
    ...
    def run_script(self) -> None:
        ...
        try:
            # Run with the same in-process interpreter used by preview/export
    ...
    def _show_version(self) -> None:
        try:
            messagebox.showinfo("Sürüm", "Qude 1.2")
        except Exception:
            pass

    def _publish_exe(self) -> None:
        code = self.editor.get("1.0", tk.END)
        # Prefill Desktop and filename Qude1.2.exe
        ...
        try:
            ...
            # Build args
            # Create Windows version resource file to embed version and description
            try:
                ...
                with open(version_path, "w", encoding="utf-8") as vf:
                    vf.write(
                        ...
                        "        StringStruct('CompanyName', 'Qude'),\n"
                        "        StringStruct('FileDescription', 'Qude 1.2'),\n"
                        "        StringStruct('FileVersion', '1.2.0.0'),\n"
                        "        StringStruct('InternalName', '" + app_name + "'),\n"
                        "        StringStruct('OriginalFilename', '" + app_name + ".exe'),\n"
                        "        StringStruct('ProductName', 'Qude 1.2'),\n"
                ...
                version_path = None
            args = [
                sys.executable, "-m", pyinst_module,
    ...
    # ---------- Kısayol Yollayıcı ----------
    def _init_quick_sender(self) -> None:
        try:
            win = tk.Toplevel(self.root)
            win.withdraw()
            win.overrideredirect(True)
            win.attributes('-topmost', True)
            frm = tk.Frame(win, bg="#202225", bd=1, highlightthickness=1, highlightbackground="#555")
            frm.pack(fill=tk.BOTH, expand=True)
            ent = tk.Entry(frm, bg="#1e1f22", fg="#e6e6e6", insertbackground="#e6e6e6", relief=tk.FLAT)
            ent.pack(fill=tk.X, padx=8, pady=6)
            hint = tk.Label(frm, text="Kısayol Yollayıcı — Enter: satırı editöre ekle, Esc: kapat (F1)", bg="#202225", fg="#aab2bf")
            ...
            ent.bind('<Escape>', on_esc)

            self.quick_win = win
            self.quick_entry = ent
        except Exception:
            self.quick_win = None
            self.quick_entry = None

    def _toggle_quick_sender(self) -> None:
        if not self.quick_win or not self.quick_entry:
            return
        try:
            if self.quick_win.state() == 'withdrawn':
                # position top-center of root
                ...
                x = rx + (rw - ww) // 2
                y = ry + 8
                self.quick_win.geometry(f"{ww}x{wh}+{x}+{y}")
                self.quick_win.deiconify()
                self.quick_win.lift()
                self.quick_entry.focus_set()
                try:
                    self.quick_win.attributes('-topmost', True)
                    self.quick_win.after(300, lambda: self.quick_win and self.quick_win.attributes('-topmost', False))
                except Exception:
                    pass
            else:
                self.quick_win.withdraw()
        except Exception:
            pass

    def _quick_insert(self) -> None:
        if not self.quick_entry or not self.quick_win:
            return
        text = self.quick_entry.get().rstrip()
        if not text:
            return
        try:
            idx = self.editor.index(tk.INSERT)
            if text == "-qs":
                self.editor.insert(idx, "Qude.prompt\n\nQude.kill/\n")
                try:
                    base_line = int(float(idx.split(".")[0]))
                except Exception:
                    base_line = int(self.editor.index(tk.INSERT).split(".")[0])
                target = f"{base_line+1}.0"
                self.editor.mark_set(tk.INSERT, target)
            elif text == "-qw":
                content = self.editor.get("1.0", tk.END)
                lines = content.splitlines()
                start_tokens = {"Qude.prompt", "qude.str()", "q>"}
                stop_tokens = {"Qude.kill/", "qude.end", "q<"}
                start_line_idx = None
                stop_line_idx = None
                for i, ln in enumerate(lines, start=1):
                    if start_line_idx is None and ln.strip() in start_tokens:
                        start_line_idx = i
                    elif start_line_idx is not None and ln.strip() in stop_tokens:
                        stop_line_idx = i
                        break
                snippet = (
                    "Qwindow.qoll()\n"
                    "Qwindow.uptext('unnamed app')\n"
                    "Qwindow.geometry.size(300, 300)\n"
                    "Qwindow.background.color('gray')\n"
                    "Qwindow.resizable = true\n"
                )
                if start_line_idx is not None and stop_line_idx is not None and start_line_idx < stop_line_idx:
                    insert_index = f"{start_line_idx+1}.0"
                    self.editor.insert(insert_index, snippet)
                    self.editor.mark_set(tk.INSERT, insert_index)
                else:
                    self.editor.insert(idx, snippet)
            elif text == "-qb":
                content = self.editor.get("1.0", tk.END)
                lines = content.splitlines()
                start_tokens = {"Qude.prompt", "qude.str()", "q>"}
                ...
                snippet = (
                    ...
                    "button1.text('button')\n"
                    "button1.size = 12\n"
                    "button1.geometry.size(120, 36)\n"
                    "button1.cordinates(40, 60)\n"
                    "button1.background.color('#444')\n"
    ...
    def _setup_editor_highlight(self) -> None:
        ...
        self.editor.tag_configure("bool", foreground="#66d9ef")
        ...
        tokens = [
            ...
            "resizable", "fullscreen", "reszbl", "fls",
            # Yazı (Label)
    ...
    def _on_edit_modified(self, _evt=None) -> None:
        # reset modified flag and debounce highlighting
        ...
        try:
            ...
            pass
        self._highlight_job = self.root.after(120, self._highlight_all)

    def _highlight_all(self) -> None:
        text = self.editor.get("1.0", tk.END)
        # Clear existing tags
        clear_tags = ["fn", "num", "str", "bool"]
        ...
        for m in re.finditer(r"(?i)\b(?:true|false)\b", text):
            self._tag_range(m.start(), m.end(), "bool")
